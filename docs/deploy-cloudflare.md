...
The generated `agentic_harness_app.js` is a stub unless you pass
`--worker-app`, `--worker-wasm`, or `--worker-wasm-crate`. A production Worker
must replace that file with an adapter that exports `invokeAgenticHarnessAgent`.
